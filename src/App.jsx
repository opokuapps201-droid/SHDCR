...
const SEED_STUDENTS = [
  { id:"S001", full_name:"Ahmed Ali Hassan",    class:"Grade 5A", dob:"2013-04-12", guardian_name:"Ali Hassan",     guardian_phone:"+20123456789", photo:null, approved:true },
  { id:"S002", full_name:"Nour Mohamed Saad",   class:"Grade 5A", dob:"2013-08-23", guardian_name:"Mohamed Saad",   guardian_phone:"+20198765432", photo:null, approved:true },
  { id:"S003", full_name:"Sara Khalid Amin",    class:"Grade 5A", dob:"2013-02-14", guardian_name:"Khalid Amin",    guardian_phone:"+20111234567", photo:null, approved:true },
  { id:"S004", full_name:"Yara Ibrahim Khalil", class:"Grade 6B", dob:"2012-01-05", guardian_name:"Ibrahim Khalil", guardian_phone:"+20112345678", photo:null, approved:true },
  { id:"S005", full_name:"Omar Samir Nour",     class:"Grade 6B", dob:"2012-11-17", guardian_name:"Samir Nour",     guardian_phone:"+20187654321", photo:null, approved:true },
  { id:"S006", full_name:"Laila Hossam Wafi",   class:"Grade 4C", dob:"2014-06-30", guardian_name:"Hossam Wafi",   guardian_phone:"+20155566677", photo:null, approved:true },
  { id:"S007", full_name:"Kareem Tarek Zaki",   class:"Grade 4C", dob:"2014-03-19", guardian_name:"Tarek Zaki",    guardian_phone:"+20144433322", photo:null, approved:true },
  { id:"S008", full_name:"Dina Amr Farouk",     class:"Grade 5A", dob:"2013-07-08", guardian_name:"Amr Farouk",    guardian_phone:"+20166677788", photo:null, approved:true },
  { id:"S009", full_name:"Ziad Hassan Younis",  class:"Grade 6B", dob:"2012-05-22", guardian_name:"Hassan Younis", guardian_phone:"+20177788899", photo:null, approved:true },
  { id:"S010", full_name:"Mariam Tarek Sobhi",  class:"Grade 4C", dob:"2014-09-11", guardian_name:"Tarek Sobhi",   guardian_phone:"+20188899900", photo:null, approved:true },
];
...
const StudentsScreen = ({user,students,attendance,isHead=false}) => {
  ...
  ,[students,selClass,search,sortBy,attendance]);
  return(
    <div className="fu">
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:22,flexWrap:"wrap",gap:12}}>
        <H children={isHead?"All Students":`${user.class} — Students`} sub={`${list.length} students`}/>
      </div>
      <div style={{display:"flex",gap:10,marginBottom:18,flexWrap:"wrap"}}>
  ...
  return(
    <div className="fu">
      ...
      <Card style={{marginBottom:24}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
          <div style={{gridColumn:"1/-1",display:"flex",alignItems:"center",gap:16,background:"#060d1a",borderRadius:14,padding:14,border:"1px dashed #1e293b",cursor:"pointer"}} onClick={()=>fileRef.current.click()}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:"#475569",marginTop:2}}>Click to upload from camera or gallery (optional, max 5MB)</div>
            </div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:12,background:"#060d1a",borderRadius:10,padding:"12px 14px",border:"1px solid #1e293b"}}>
            <div>
              <div style={{fontSize:10,color:"#475569",textTransform:"uppercase",letterSpacing:.8}}>Calculated Age</div>
              <div style={{fontSize:22,fontWeight:800,color:"#38bdf8",fontFamily:"'Syne',sans-serif",marginTop:2}}>{form.dob?`${calcAge(form.dob)} yrs`:"—"}</div>
            </div>
          </div>
          <div style={{display:"flex",flexDirection:"column",gap:4}}>
            <Inp label="Guardian Name *" value={form.guardian_name} onChange={e=>setF("guardian_name",e.target.value)} placeholder="Parent or guardian"/>
          ...
          <div style={{gridColumn:"1/-1",display:"flex",justifyContent:"flex-end"}}>
            <Btn onClick={submit} size="lg">Submit for Approval →</Btn>
          </div>
        </div>
      </Card>
...
// TEACHERS
// ═══════════════════════════════════════════════════════════════
const TeachersScreen = ({students,attendance}) => (
  <div className="fu">
    <H children="Teachers" sub={`${USERS.filter(u=>u.role==="teacher").length} registered`}/>
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
          ...
          <Card key={t.id} className="card-hover">
            <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
              ...
              <div>
                ...
                <div style={{fontSize:12,color:"#475569",marginTop:4}}>📚 {t.class}</div>
              </div>
            </div>
...
// CHAT
// ═══════════════════════════════════════════════════════════════
const ChatScreen = ({user,messages,setMessages}) => {
  ...
  return(
    <div style={{display:"flex",height:"calc(100vh - 80px)",gap:12}}>
      <div style={{width:200,background:"#040810",borderRadius:14,border:"1px solid #0f2040",overflow:"hidden",display:"flex",flexDirection:"column",flexShrink:0}}>
        <div style={{padding:"12px 14px",borderBottom:"1px solid #0a1628",fontSize:10,fontWeight:700,color:"#334155",textTransform:"uppercase",letterSpacing:1}}>Messages</div>
        {partners.map(p=>{
          ...
          return(
            <button key={p.id} onClick={()=>setSelId(p.id)}
              ...
              <div style={{flex:1,minWidth:0}}>
                <div style={{fontSize:12,fontWeight:600,color:"#e2e8f0",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{p.name.split(" ").slice(1).join(" ")}</div>
      ...
      <div style={{flex:1,display:"flex",flexDirection:"column",background:"#0d1829",borderRadius:14,border:"1px solid #0f2040",overflow:"hidden",minWidth:0}}>
        {selUser?(
          <>
            <div style={{padding:"12px 18px",borderBottom:"1px solid #0a1628",display:"flex",alignItems:"center",gap:10}}>
              <Av label={selUser.avatar} color={RC[selUser.role]} size={36}/>
              <div style={{flex:1}}>
                <div style={{fontWeight:700,fontSize:14,color:"#e2e8f0"}}>{selUser.name}</div>
                <RolePill role={selUser.role}/>
              </div>
              {user.role==="director"&&<span style={{fontSize:10,color:"#334155",background:"#060d1a",padding:"3px 10px",borderRadius:99,border:"1px solid #0f2040"}}>🔒 Director ↔ Headmaster only</span>}
            </div>
              ...
              {thread.map(m=>{
                ...
                return(
                  <div key={m.id} style={{display:"flex",justifyContent:mine?"flex-end":"flex-start",gap:8,alignItems:"flex-end"}}>
                    ...
                    <div style={{maxWidth:"65%",padding:"9px 14px",borderRadius:mine?"16px 16px 4px 16px":"16px 16px 16px 4px",background:mine?"linear-gradient(135deg,#0ea5e9,#0369a1)":"#1e293b",color:"#f1f5f9",fontSize:13,lineHeight:1.5,wordBreak:"break-word"}}>
                      <div>{m.text}</div>
            ...
            <div style={{padding:"10px 14px",borderTop:"1px solid #0a1628",display:"flex",gap:8}}>
              <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder={`Message ${selUser.name.split(" ")[1]}…`}
                style={{flex:1,background:"#060d1a",border:"1px solid #1e293b",borderRadius:10,padding:"9px 14px",color:"#e2e8f0",fontSize:13,outline:"none"}}/>
              <Btn onClick={send} disabled={!input.trim()}>Send</Btn>
            </div>
          </>
        ):<div style={{display:"flex",alignItems:"center",justifyContent:"center",flex:1,color:"#334155",fontSize:13}}>Select a conversation</div>}
      </div>
    </div>
  );
};
...
export default function App() {
  ...
  const pending=requests.filter(r=>r.status==="pending").length;
  if(!user)return<><GS/><Login onLogin={u=>{setUser(u);setScreen("dashboard");}}/></>;
  const SW=collapsed?64:220;
  const sp={user,students,setStudents,attendance,setAttendance,requests,setRequests,messages,setMessages,announcements,setAnnouncements,smsLog,setSmsLog,actLog,setActLog};
  const renderScreen=()=>{
    const r=user.role;
    switch(screen){
      ...
      case"teachers":     return<TeachersScreen {...sp}/>;
      case"classes":      return<ClassesScreen {...sp}/>;
      case"chat":         return<ChatScreen {...sp}/>;
